                ...
                if current_time >= time_recovery_threshold:
                    ...
                    if not no_llm:
                        try:
                            ...
                            log.warning(f"[LLM ENTRY ERROR] Validation error: {e}. Proceeding with confirmed picks.")
                    # ───────────────────────────────────────────────────────
                    ...
                    try:
                        ...
                        if remaining_capital < 1000:
                            ...
                            if not re_picks:
                                ...
                                # 7. Sentiment confirmation (With Sentiment variant)
                                try:
                                    re_sentiment_engine = SentimentDecisionEngine(market=market)
                                    if re_sentiment_engine.enabled:
                                        re_picks, _ = re_sentiment_engine.evaluate(re_picks, today_date)
                                except Exception as _se:
                                    log.warning(f"[HOURLY RE-SELECTION] Sentiment check failed: {_se}. Proceeding.")

                                if not re_picks:
                                    log.info("[HOURLY RE-SELECTION] All new picks filtered out by sentiment.")
                                else:
                                    # 8. LLM entry validation (With LLM variant)
                                    if not no_llm:
                                        try:
